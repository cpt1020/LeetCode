...
/*
g++ 851.cpp ../utils.cpp -std=c++17 -o 851 && ./851
*/

class Solution {
// Topological Sort
public:
    vector<int> loudAndRich_TopologicalSort(vector<vector<int>>& richer, vector<int>& quiet) {
        int n = quiet.size();
        ...
        for (auto& vec: richer) {
            richerThan.at(vec.at(0)).push_back(vec.at(1));
            inDegree.at(vec.at(1)) += 1;
        }

        vector<pair<int, int>> leastQuietPerson (n);
        // first is the quietness of the least quiet person
        // second is the index of the least quiet person
        for (int i {0}; i < n; ++i) {
            leastQuietPerson.at(i) = {quiet.at(i), i};
        }
        ...
        for (int i {0}; i < n; ++i) {
            ...
            for (auto k: richerThan.at(j)) {
                inDegree.at(k) -= 1;
                if (leastQuietPerson.at(j).first < leastQuietPerson.at(k).first) {
                    leastQuietPerson.at(k).first = leastQuietPerson.at(j).first;
                    leastQuietPerson.at(k).second = leastQuietPerson.at(j).second;
                }
            }

            ans.at(j) = leastQuietPerson.at(j).second;
        }
...
// DFS (not optimized)
private:
    void dfs(vector<vector<int>>& lessThan, vector<int>& quiet, int& i, int& leastQuietPerson) {
        if (quiet.at(i) < quiet.at(leastQuietPerson)) {
            leastQuietPerson = i;
        }

        for (auto per: lessThan.at(i)) {
            dfs(lessThan, quiet, per, leastQuietPerson);
        }
    }
public:
    vector<int> loudAndRich_DFS(vector<vector<int>>& richer, vector<int>& quiet) {
        int n = quiet.size();

        vector<int> ans (n, 0);
        vector<vector<int>> lessThan (n);
        for (auto& vec: richer) {
            lessThan.at(vec.at(1)).push_back(vec.at(0));
        }

        for (int i {0}; i < n; ++i) {
            int leastQuietPerson {i};
            dfs(lessThan, quiet, i, leastQuietPerson);
            ans.at(i) = leastQuietPerson;
        }
...
// DFS (optimized)
private:
    int dfs_optimized(vector<vector<int>>& lessThan, int& i, vector<int>& ans, vector<int>& quiet) {
        if (ans.at(i) != -1) {
            return ans.at(i);
        }
        else {
            int curLeastQuietPerson {i};
            for (auto person: lessThan.at(i)) {
                int leastQuietPersonOfPerson {dfs_optimized(lessThan, person, ans, quiet)};
                if (quiet.at(leastQuietPersonOfPerson) < quiet.at(curLeastQuietPerson)) {
                    curLeastQuietPerson = leastQuietPersonOfPerson;
                }
            }
            ans.at(i) = curLeastQuietPerson;
            return curLeastQuietPerson;
        }
    }
public:
    vector<int> loudAndRich_DFS_optimized(vector<vector<int>>& richer, vector<int>& quiet) {
        ...
        vector<int> ans (n, -1);
        for (int i {0}; i < n; ++i) {
            dfs_optimized(lessThan, i, ans, quiet);
        }
...
int main() {
    ...
    printVector(sol.loudAndRich_TopologicalSort(richer, quiet));
    printVector(sol.loudAndRich_DFS_optimized(richer, quiet));
    printVector(sol.loudAndRich_DFS(richer, quiet));
